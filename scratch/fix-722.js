...
async function processDb(prisma, label) {
  ...
  if (!account) {
    console.log('lucky cement limited account not found.');
    return;
  }
  ...
  // 2. Print entry 722 before
  const entryBefore = await prisma.ledger.findUnique({
    where: { l_id: 722 }
  });
  if (!entryBefore) {
    console.log('Ledger entry 722 not found.');
    return;
  }
  console.log('Entry 722 before modification:');
  console.log(JSON.stringify(entryBefore, null, 2));

  // 3. Update entry 722
  console.log('Updating entry 722...');
  ...
  console.log('Entry 722 updated successfully.');
  ...
  for (const entry of entries) {
    ...
    // If it's the updated entry or subsequent ones, we perform the update in DB
    if (
      Math.abs(currentOpening - opening) > 0.01 ||
      Math.abs(currentClosing - closing) > 0.01 ||
      entry.l_id === 722 // force update of entry 722 to ensure correct closing/opening
    ) {
      await prisma.ledger.update({

...
async function processDb(prisma, label) {
  ...
  if (!account) {
    console.log('MALIK ARIF SB LAHORE account (ID 2562) not found.');
    return;
  }
  ...
  // 2. Print entry 614 before
  const entryBefore = await prisma.ledger.findUnique({
    where: { l_id: 614 }
  });
  if (!entryBefore) {
    console.log('Ledger entry 614 not found.');
    return;
  }
  console.log('Entry 614 before modification:');
  console.log(JSON.stringify(entryBefore, null, 2));

  // 3. Update entry 614
  console.log('Updating entry 614...');
  ...
  console.log('Entry 614 updated successfully.');

...
async function processDb(prisma, label, isLive) {
  console.log(`\n==================================================`);
  ...
  // 1. Revert specific entries: 726, 728, 730 to Credit entries
  console.log('Reverting ledger entries 726, 728, 730 to CREDIT...');
  ...
  for (const target of targets) {
    await prisma.ledger.update({
      ...
      data: {
        ...
        trnx_type: isLive ? 'CREDIT' : 'CASH_PAYMENT',
        ledger_type: isLive ? 'Receiving' : 'Payment'
      }
    });
    console.log(`  Reverted entry ${target.id}: Credit set to ${target.amount}.`);
  }

  // 2. Recalculate balances
  console.log('Recalculating ledger balances (credit entry subtracts from balance)...');
  ...
  for (const entry of entries) {
    ...
    // We update every entry to ensure the math is strictly verified
    await prisma.ledger.update({
...
async function main() {
  try {
    await processDb(dbLocal, 'Office DB (72.60.76.68)', false);
    await processDb(dbLive, 'Live DB (195.35.59.84)', true);
  } catch (error) {

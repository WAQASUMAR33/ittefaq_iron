...
async function processDb(prisma, label, url) {
  ...
  for (const id of creditIds) {
    ...
    console.log(`  Set entry ${id} (Cus_ID: ${entry.cus_id}) to CREDIT (${amount}).`);
  }
  ...
  for (const id of debitIds) {
    ...
    console.log(`  Set entry ${id} (Cus_ID: ${entry.cus_id}) to DEBIT (${amount}).`);
  }
  ...
  for (const cusId of affectedCusIds) {
    ...
    // Update customer balance in customer table

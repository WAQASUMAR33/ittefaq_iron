...
/**
 * Universal Payment Receipt Voucher Component.
 * Recalled in Ledger Page (Finance) for Payment Transactions.
 */
const PaymentReceipt = React.memo(function PaymentReceipt({
  voucherData,
  currentUser,
  ...
  return (
    <Box id={elementId} className={className} sx={{ bgcolor: 'white', p: 3, color: '#000', fontWeight: 'bold', '& *': { fontWeight: 'bold !important', color: '#000 !important' } }}>
      ...
      <Box sx={{ textAlign: 'center', pb: 2, borderBottom: '2px solid #000', mb: 2 }}>
        ...
        <Typography variant="h6" sx={{ fontWeight: 'bold', mt: 1, letterSpacing: 1, color: '#dc2626' }}>
          PAYMENT VOUCHER (ادائیگی کی رسید)
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2, px: 1 }}>
        <Box>
          <Typography variant="body2"><strong>Account / Paid To:</strong> {voucherData.customer?.cus_name || voucherData.accountName || 'N/A'}</Typography>
        ...
        <Box sx={{ textAlign: 'right' }}>
          <Typography variant="body2"><strong>Ref #:</strong> PAY-{voucherData.paymentId || voucherData.id || 'N/A'}</Typography>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 4, pt: 2 }}>
        <Box sx={{ textAlign: 'center', flex: 1 }}>
          <Box sx={{ borderTop: '1px solid #000', pt: 0.5, mx: 3 }}>
            <Typography variant="caption">Paid By</Typography>
          </Box>
        </Box>
        <Box sx={{ textAlign: 'center', flex: 1 }}>
          <Box sx={{ borderTop: '1px solid #000', pt: 0.5, mx: 3 }}>

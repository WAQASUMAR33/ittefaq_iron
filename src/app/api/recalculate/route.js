...
export async function GET() {
  console.log('=== Vercel-side force CREDIT for target Mezaan bank entries ===');
  const results = [];
  const targetIds = [1585, 1588];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      results.push(`Forced entry ${entry.l_id} to CREDIT (Payment) with amount ${amount}`);
    }
    ...
    console.error('Force CREDIT API error:', error);

...
export default function PurchasesBySupplierReport() {
  ...
  const handleExport = () => {
    ...
    reportData.supplierPurchases.forEach(sp => {
      csv += `${sp.supplier?.cus_name || 'N/A'},${sp.purchasesCount},${fmtAmt(sp.totalAmount)},${fmtAmt(sp.totalUnloading)},${fmtAmt(sp.totalFare)},${fmtAmt(sp.totalDiscount)},${fmtAmt(sp.netTotal)},${fmtAmt(sp.totalPayment)}\n`;
    });

    csv += '\n';
    csv += `TOTAL,${reportData.summary.totalPurchases},${fmtAmt(reportData.summary.totalAmount)},${fmtAmt(reportData.summary.totalUnloading)},${fmtAmt(reportData.summary.totalFare)},${fmtAmt(reportData.summary.totalDiscount)},${fmtAmt(reportData.summary.netTotal)},${fmtAmt(reportData.summary.totalPayment)}\n`;

    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `purchases-by-supplier-${startDate}-to-${endDate}.csv`;
    a.click();
        ...
        <div className="flex-shrink-0 mb-6 print:hidden">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Purchase Report (Supplier Wise)</h2>
                <p className="text-gray-600 mt-1">View purchases grouped by suppliers</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex-1 min-h-0 print:min-h-0 print:block">
              <div className="bg-white rounded-lg shadow print:shadow-none print:border print:border-gray-300 h-full flex flex-col print:block">
                <div className="flex-shrink-0 px-6 py-4 border-b border-gray-200">
                  <h3 className="text-lg font-semibold text-gray-900">Supplier Purchase Summary</h3>
                </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 print:block print:table">
                    <thead className="bg-gray-50 sticky top-0 print:relative">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Purchase Count</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {reportData.supplierPurchases.map((sp) => (
                        <tr key={sp.supplier?.cus_id} className="hover:bg-gray-50 print:hover:bg-white">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <div className="font-medium">{sp.supplier?.cus_name || 'N/A'}</div>
                            <div className="text-xs text-gray-500">{sp.supplier?.cus_phone_no || ''}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">
                            {sp.purchasesCount}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">
                            {fmtAmt(sp.totalAmount)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-blue-600">
                            {fmtAmt(sp.totalUnloading)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-purple-600">
                            {fmtAmt(sp.totalFare)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-red-600">
                            {fmtAmt(sp.totalDiscount)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold text-green-600">
                            {fmtAmt(sp.netTotal)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">
                            {fmtAmt(sp.totalPayment)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="bg-gray-50 font-semibold">
                      <tr>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-blue-600">
                          {fmtAmt(reportData.summary.totalUnloading)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-purple-600">
                          {fmtAmt(reportData.summary.totalFare)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-red-600">
